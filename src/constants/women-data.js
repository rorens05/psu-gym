export const WOMENDATA = {
  title: 'Women',
  image: require('../assets/images/women.jpg'),
  categories: [
    {
      title: 'WEIGHT LOSS WORKOUTS',
      image: require('../assets/images/womenworkout1.jpg'),
      type: 'WORKOUT',
      data: [
        {
          title: 'DAY 1: UPPER BODY',
          exercises: [
            {
              image: require('../assets/images/wall-push-up.gif'),
              title: 'WALL PUSH UP',
              description: `Assume the starting position with feet and legs together, standing about 2 feet from a wall with your arms straight out in front of you. ...
              Bend your elbows and begin to lean your body toward the wall until your nose almost touches it. ...
              Push back to the starting position and repeat.
              3 SET with 10-12 Reps
              `,
            },
            {
              image: require('../assets/images/jumping-jacks.gif'),
              title: 'JUMPING JACKS',
              description: `Stand upright with your legs together, arms at your sides.
                Bend your knees slightly, and jump into the air.
                As you jump, spread your legs to be about shoulder-width apart. Stretch your arms out and over
                your head.
                Jump back to starting position.
                Repeat.
                2 Set with 20-30 Reps`,
            },
            {
              image: require('../assets/images/chair-dips.gif'),
              title: 'CHAIR DIPS',
              description: `Move your torso forward off the chair with your arms extended. Your buttocks should hover over the floor and your knees should be slightly bent. Your heels should touch the floor a few inches in front of your knees. Breathe in as you slowly lower your body, hinging at the elbows until each forms a 90-degree angle.
              3 SET 2/4 REPS
              `,
            },
            {
              image: require('../assets/images/arm-circle.gif'),
              title: 'ARM CIRCLE',
              description: `Standing with a flat back and tight core, raise your arms to the sides.
              While focusing on the shoulders, slowly rotate your arms in a circular motion. Start with small circles. Gradually increase the size of the circles.
              4 SET 4/8 REPS
              `,
            },
          ],
        },
        {
          title: 'DAY 2: ABS + CORE',

          exercises: [
            {
              image: require('../assets/images/abs-and-core.gif'),
              title: 'ABS + CORE',
              description: `Your torso back about 45 degrees. This will be your starting position. Now exhale and bring your knees in toward you while simultaneously leaning your torso up toward your knees.
              3 Set 10-12 Reps
              `,
            },
            {
              image: require('../assets/images/no-name.gif'),
              title: 'CURL UP',
              description: `Start by lying on the ground, with your lower back pressed flat into the floor and your head and
              shoulders raised slightly above it.
              Place your hands lightly on the sides of your head; don&#39;t knit your fingers behind. ...
              Lift one leg just off the ground and extend it out.
              2 Set with 10-12 Reps.`,
            },
            {
              image: require('../assets/images/side-plank.gif'),
              title: 'SIDE PLANK',
              description: `Start on your side with your feet together and one forearm directly below your shoulder. Contract your core and raise your hips until your body is in a straight line from head to feet. Hold the position for 2 minutes without letting your hips drop for the allotted time for each set, then repeat on the other side.
              `,
            },
          ],
        },
        {
          title: 'DAY 3: REST',
          type: 'REST',
        },
        {
          title: 'DAY 4: LOWER BODY',

          exercises: [
            {
              image: require('../assets/images/squats.gif'),
              title: 'SQUATS',
              description: `Stand with your feet shoulder width apart and hold a barbell behind you at arms length. While
              keeping your head up and back straight, squat until your thighs are parallel to the floor. – Drive
              through your heels to stand back up as you breathe out.
              REPEAT FOR 3 SETS 15 REPS`,
            },
            {
              image: require('../assets/images/side-plank-2.gif'),
              title: 'SIDE PLANK 2',
              description: `Start with your hands out in front of you or resting on your hips. ...
              As you lift your right leg up off the floor with the foot flexed, inhale and shift the weight into your left foot.
              As you exhale, bring the leg back down to meet the left.
              Repeat 10-12 times, then switch to the other side.
              `,
            },
            {
              image: require('../assets/images/lunges.gif'),
              title: 'LUNGES',
              description: `stand tall with feet hip width apart engage core take a big step forward with right leg and start to
              shift weight forward so heel hits the floor first . Lower body until right thigh is parallel to floor and
              right shin is vertical press into right heel to drive back up to starting position .
              Repeat 3 sets with 15 reps each.`,
            },
          ],
        },
        {
          title: 'DAY 5: ABS + CORE',

          exercises: [
            {
              image: require('../assets/images/no-name.gif'),
              title: 'CURL UP',
              description: `Start by lying on the ground, with your lower back pressed flat into the floor and your head and
              shoulders raised slightly above it.
              Place your hands lightly on the sides of your head; don&#39;t knit your fingers behind. ...
              Lift one leg just off the ground and extend it out.
              2 Set with 10-12 Reps.`,
            },
            {
              image: require('../assets/images/leg-raise.gif'),
              title: 'LEG RAISE',
              description: `Lie faceup on the floor with your legs straight and your palms facing down. This is the starting position. Without bending your legs, lift them toward the ceiling until your hips just leave the floor. Then lower your legs down to the starting position again.
              2 SETS REPEAT FOR 10-12 REPS
              `,
            },
            {
              image: require('../assets/images/abs-and-core.gif'),
              title: 'ABS + CORE',
              description: `Your torso back about 45 degrees. This will be your starting position. Now exhale and bring your knees in toward you while simultaneously leaning your torso up toward your knees.
              3 Set 10-12 Reps
              `,
            },
          ],
        },
        {
          title: 'DAY 6: CARDIO',
          exercises: [
            {
              image: require('../assets/images/run.gif'),
              title: 'RUN',
              description: `Jogging is a form of aerobic exercise in which you maintain a running speed under 6 mph. Regularly jogging can help you lose weight, especially if you also modify your diet.
              `,
            },
          ],
        },
        {
          title: 'DAY 7: REST',
          type: 'REST',
        },
      ],
    },
    {
      title: 'WEIGHT LOSS DIET',
      image: require('../assets/images/weight_loss_2.jpg'),
      type: 'DIET',
      data: `
WEIGHT LOSS FOR WOMEN


Repeat the following diet for a period of four weeks for best results.

Day 1

Breakfast:
1 Serving Avocado-Egg Toast (271 calories)

A.M Snack:
1 medium orange (62 calories)

Lunch: 
1 Serving Butternut Squash Soup with Avocado & Chickpeas (402 calories)

P.M. Snack:
1 medium kiwi (42 calories)

Dinner:
1 Serving Citrus Poached Salmon with Asparagus with 3/4 cup Cauliflower Rice (451 calories)


Day 2

Breakfast:
1 Serving Avocado-Egg Toast (271 calories)

A.M Snack:
1 medium orange (62 calories)

Lunch: 
1 Serving Butternut Squash Soup with Avocado & Chickpeas (402 calories)

P.M. Snack:
1 medium kiwi (42 calories)

Dinner:
1 Serving Citrus Poached Salmon with Asparagus with 3/4 cup Cauliflower Rice (451 calories)


Day 3

Breakfast:
1 Serving Avocado-Egg Toast (271 calories)

A.M Snack:
1 medium orange (62 calories)

Lunch: 
1 Serving Butternut Squash Soup with Avocado & Chickpeas (402 calories)

P.M. Snack:
1 medium kiwi (42 calories)

Dinner:
1 Serving Citrus Poached Salmon with Asparagus with 3/4 cup Cauliflower Rice (451 calories)


Day 4

Breakfast:
1 Serving Avocado-Egg Toast (271 calories)

A.M Snack:
1 medium orange (62 calories)

Lunch: 
1 Serving Butternut Squash Soup with Avocado & Chickpeas (402 calories)

P.M. Snack:
1 medium kiwi (42 calories)

Dinner:
1 Serving Citrus Poached Salmon with Asparagus with 3/4 cup Cauliflower Rice (451 calories)


Day 5

Breakfast:
1 Serving Avocado-Egg Toast (271 calories)

A.M Snack:
1 medium orange (62 calories)

Lunch: 
1 Serving Butternut Squash Soup with Avocado & Chickpeas (402 calories)

P.M. Snack:
1 medium kiwi (42 calories)

Dinner:
1 Serving Citrus Poached Salmon with Asparagus with 3/4 cup Cauliflower Rice (451 calories)


Day 6

Breakfast:
1 Serving Avocado-Egg Toast (271 calories)

A.M Snack:
1 medium orange (62 calories)

Lunch: 
1 Serving Butternut Squash Soup with Avocado & Chickpeas (402 calories)

P.M. Snack:
1 medium kiwi (42 calories)

Dinner:
1 Serving Citrus Poached Salmon with Asparagus with 3/4 cup Cauliflower Rice (451 calories)


Day 7

Breakfast:
1 Serving Avocado-Egg Toast (271 calories)

A.M Snack:
1 medium orange (62 calories)

Lunch: 
1 Serving Butternut Squash Soup with Avocado & Chickpeas (402 calories)

P.M. Snack:
1 medium kiwi (42 calories)

Dinner:
1 Serving Citrus Poached Salmon with Asparagus with 3/4 cup Cauliflower Rice (451 calories)
`,
    },
    {
      title: 'WEIGHT GAIN WORKOUTS',
      image: require('../assets/images/womenworkout2.jpg'),
      type: 'WORKOUT',
      data: [
        {
          title: 'DAY 1: BASICS',
          exercises: [
            {
              image: require('../assets/images/arm-circle.gif'),
              title: 'ARM CIRCLE',
              description: `Standing with a flat back and tight core, raise your arms to the sides.
              While focusing on the shoulders, slowly rotate your arms in a circular motion. Start with small circles. Gradually increase the size of the circles.
              4 SET 4/8 REPS
              `,
            },
            {
              image: require('../assets/images/jumping-jacks.gif'),
              title: 'JUMPING JACKS',
              description: `Stand upright with your legs together, arms at your sides.
                Bend your knees slightly, and jump into the air.
                As you jump, spread your legs to be about shoulder-width apart. Stretch your arms out and over
                your head.
                Jump back to starting position.
                Repeat.
                2 Set with 20-30 Reps`,
            },
          ],
        },
        {
          title: 'DAY 2: REPEAT THE WORKOUT ON DAY 1',
          exercises: [
            {
              image: require('../assets/images/arm-circle.gif'),
              title: 'ARM CIRCLE',
              description: `Standing with a flat back and tight core, raise your arms to the sides.
              While focusing on the shoulders, slowly rotate your arms in a circular motion. Start with small circles. Gradually increase the size of the circles.
              4 SET 4/8 REPS
              `,
            },
            {
              image: require('../assets/images/jumping-jacks.gif'),
              title: 'JUMPING JACKS',
              description: `Stand upright with your legs together, arms at your sides.
                Bend your knees slightly, and jump into the air.
                As you jump, spread your legs to be about shoulder-width apart. Stretch your arms out and over
                your head.
                Jump back to starting position.
                Repeat.
                2 Set with 20-30 Reps`,
            },
          ],
        },
        {
          title: 'DAY 3: CORE',
          exercises: [
            {
              image: require('../assets/images/plank.gif'),
              title: 'PLANK',
              description: `Get into pushup position on the floor
              Now bend your elbows 90° and rest your weight on your forearms. Your elbows should be
              directly beneath your shoulders, and your body should form a straight line from your head to
              your feet. Hold the position for as long as you can. Your goal should be to hold it for two
              minutes.
              `,
            },
            {
              image: require('../assets/images/squats.gif'),
              title: 'SQUATS',
              description: `Stand with your feet shoulder width apart and hold a barbell behind you at arms length. While
              keeping your head up and back straight, squat until your thighs are parallel to the floor. – Drive
              through your heels to stand back up as you breathe out.
              REPEAT FOR 3 SETS 15 REPS`,
            },
            {
              image: require('../assets/images/no-name.gif'),
              title: 'CURL UP',
              description: `Start by lying on the ground, with your lower back pressed flat into the floor and your head and
              shoulders raised slightly above it.
              Place your hands lightly on the sides of your head; don&#39;t knit your fingers behind. ...
              Lift one leg just off the ground and extend it out.
              2 Set with 10-12 Reps.`,
            },
          ],
        },
        {
          title: 'DAY 4: REST',
          type: 'REST',
        },
        {
          title: 'DAY 5: STRENGTH',
          exercises: [
            {
              image: require('../assets/images/wall-push-up.gif'),
              title: 'WALL PUSH UP',
              description: `Assume the starting position with feet and legs together, standing about 2 feet from a wall with
              your arms straight out in front of you. ...
              Bend your elbows and begin to lean your body toward the wall until your nose almost touches it.
              ...
              Push back to the starting position and repeat.
              3 SET with 10-12 Reps
              `,
            },
            {
              image: require('../assets/images/chair-dips.gif'),
              title: 'CHAIR DIPS',
              description: `Move your torso forward off the chair with your arms extended. Your buttocks should hover over
              the floor and your knees should be slightly bent. Your heels should touch the floor a few inches
              in front of your knees. Breathe in as you slowly lower your body, hinging at the elbows until each
              forms a 90-degree angle.
              3 SET 2/4 REPS`,
            },
            {
              image: require('../assets/images/lunges.gif'),
              title: 'LUNGES',
              description: `stand tall with feet hip width apart engage core take a big step forward with right leg and start to
              shift weight forward so heel hits the floor first . Lower body until right thigh is parallel to floor and
              right shin is vertical press into right heel to drive back up to starting position .
              Repeat 3 sets with 15 reps each.`,
            },
          ],
        },
        {
          title: 'DAY 6: REST',
          type: 'REST',
        },
        {
          title: 'DAY 7: CARDIO',
          exercises: [
            {
              image: require('../assets/images/run.gif'),
              title: 'RUN',
              description: `Jogging is a form of aerobic exercise in which you maintain a running speed under 6 mph. Regularly jogging can help you lose weight, especially if you also modify your diet.
              `,
            },
          ],
        },
      ],
    },
    {
      title: 'WEIGHT GAIN DIET',
      image: require('../assets/images/weight_gain_2.jpg'),
      type: 'DIET',
      data: `
      WEIGHT GAIN FOR WOMEN
      
      Follow the diet for a period of two months to have the best results.
      
      Day 1
      
      Breakfast
      For breakfast, have a bowl of oatmeal with whole milk, dried fruits and nuts.You can have a glass of orange juice with it.
      
      Snack 
      Have a cereal bar with banana as your snack.
      
      Lunch 
      For lunch, it is necessary to have heavy food items.You can have baked potatoes with tuna with a generous amount of grated cheese on top . A portion of fruit gelatin and heavy dessert can also be eaten eith this.Also, make sure that you have an apple and a few squares of dark chocolate with your lunch.
      
      Dinner
      For your dinner, you can have a potion of grilled chicken with avocado salad whole grain bread.You can also add olive oil to your food.For dessert have a bowl of yoghurt and top it with seasonal fruits.
      
      Snack
      Post-dinner snack is a very impotant partof this weight gain diet.Have a glass of vanilla smoothie made out of whole milk
      
      DAY 2
      
      Breakfast 
      for the second-day breakfast have two scrambled eggs with whole milk.Serve your breakfast with two slices of whole grain bread and a glass of fresh orange juice 
      
      Snack 
      Have a bowl of yogurt with peaches.
      
      Lunch
      Healthy foods to gain weight fast.Prepare a palte of chicken pasta salad and have it as your lunch on day 2.Additionally, have a quarter bar of dark chocolate with it.
      
      Snack
      For an evening snack have a sliced avocado with whole grain crackers.
      
      Dinner
      Meal like stir-fried turkey cooked using vegetable oil with onions, tomatoes and peppers served with brown rice perfect for the day 2 dinner.With this have a banana for dessert
      
      Snack							Drink a glass of whole milk with a cookie for a post-dinner snack.
      
      DAY 3
                      Breakfast 
      For day 3 breakfast have a bowl of whole grain cereal with whole milk and top it with sliced mango.
      
      Snack
      Have crackers topped with some mashed banana for your snack.
      
      Lunch 
      Have the leftover turkey from the previous day as your lunch with an apple.
      
      Snack 
      In the evening, have a packet of trial mix and top it with a genereous amount of cheese.
      
      Dinner 
      Nuts & dry fruits to gain weight for dinner have grilled tilapia with sweet potatoes, roasted bell peppers and some couscous.Also, have a one glass of fresh orange juice.
      
      Snack
      Prepare a strawberry smootie using whole milk and have it as your post-dinner snack.
      
      DAY 4
      
      Breakfast
      For breakfast, have a bowl of oatmeal with whole milk, dried fruits and nuts.You can have a glass of orange juice with it.
      
      Snack 
      Have a cereal bar with banana as your snack.
      
      Lunch 
      For lunch, it is necessary to have heavy food items.You can have baked potatoes with tuna with a generous amount of grated cheese on top . A portion of fruit gelatin and heavy dessert can also be eaten eith this.Also, make sure that you have an apple and a few squares of dark chocolate with your lunch.
      
      Dinner
      For your dinner, you can have a portion of salmon with avocado salad and whole grain bread.You can also add olive oil to your food.For dessert have a bowl of youghurt and top it with seasonal fruits.
      
      Snack
      Post-dinner snack is a very impotant partof this weight gain diet.Have a glass of vanilla smoothie made out of whole milk.
      
      DAY 5
      
      Breakfast 
      A hard-boiled egg with two toast slices and fresh mango juice.
      
      Snack
      Have a packet of low-salt potato chips with a peach.
      
      Lunch
      Have a ham-salad sandwich made out of whole grain bread, yogurt and two squares of dark chocolate.
      
      Snack
      Prepare a banana and peanut smoothie with whole milk and have it as snack post lunch.
      
      Dinner 
      For dinner cook some salmon and prawns in using vegetable oil and serve it with whole wheat pasta and tomato sauce.Also, have some fresh fruit salad with it.
      
      Snack 
      Drink a glass of whole milk with a cookie.
      
      DAY 6
      
      Breakfast
      For breakfast have a bowl of whole grain cereal with whole milk and top it with sliced mango.
      
      Snack
      Have crackers topped with some mashed banana for your snack.
      
      Lunch 
      Have the leftover turkey from the previous day as your lunch with an apple.
      
      Snack
      In the evening, have a packet of trail mix and top it with a generous amount of cheese.
      
      Dinner 
      Nuts & dry fruits to gain weight for dinner have grilled tilapia with sweet potatoes, roasted bell peppers and some couscous.Also,have a one glass of fresh orange juice.
      
      Snack
      Prepare a strawberry smoothie using whole milk and have it as your post-dinner snack.
      
      DAY 7 
      
      Breakfast
      For the breakfast on the final day have oatmeal cooked using whole milk topped with freash mangoes and chopped almonds.
      
      Snack
      Prepare a mix of curd, raisins and cereal bar and have it as your snack.
      
      Lunch 
      Cook a peanut butter sandwich with whoe grain bread.Drink a glass  of fresh juice and cookie with the sandwich with whole grain bread.Drink a glass of fresh juice and cookie with the sandwich.
      
      Snack
      Have crackers with mashed avocado.
      
      Dinner
      Prepare a salad with lentils, mackerel and anchovies, drizzle some olive oil on it and have it with whole grain bread as your dinner
      
      Snack
      Drink a banana smoothie made using whole milk.
      
`,
    },
  ],
};
